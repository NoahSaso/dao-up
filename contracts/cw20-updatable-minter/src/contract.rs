...
pub fn execute_update_minter(
    ...
    // The cap must remain unchanged so that a minter can not update
    // minter to be themselves and increase the cap.
    let cap = config.mint.unwrap().cap;
    config.mint = Some(MinterData { minter, cap });
    TOKEN_INFO.save(deps.storage, &config)?;

    let res = Response::new()
        .add_attribute("action", "update_minter")
        .add_attribute("minter", config.mint.as_ref().unwrap().minter.clone());
    Ok(res)
}
...
mod tests {
    ...
    mod instantiate {
        ...
        fn update_minter() {
            ...
            let cap = Uint128::new(12345);
            let minter = String::from("escrow");
            ...
            // Minter can be updated by the current minter.
            let info = mock_info("escrow", &[]);
            let env = mock_env();
            execute(
                deps.as_mut(),
                env,
            ...
            // Minter can not be updated by the owner or non minter.
            let info = mock_info("creator", &[]);
            let env = mock_env();
            execute(
                deps.as_mut(),
